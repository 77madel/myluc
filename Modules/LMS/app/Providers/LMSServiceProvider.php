...
/*    public function boot(Kernel $kernel): void
    ...
    public function boot(Kernel $kernel): void
    {
        ...
        // CORRECTION 1 : Condition pour éviter l'accès au cache/DB lors de l'exécution de commandes Artisan (comme 'migrate').
        if (!$this->app->runningInConsole()) {
            $this->app->singleton('options', function () {
                // Cette fonction utilise get_options() qui lit le cache/DB.
                return get_options();
            });
        }
        ...
        if (alreadyInstalled() && !$this->app->runningInConsole()) {
            ...
            // qui lisent la DB/cache. Elles doivent être évitées pendant la migration.
            $this->mailDriverSet();
            $this->paystackConfigure();
        }
        ...
        // Ces blocs semblent bien, mais ajoutons une condition pour get_themes() si elle appelle la DB.
        // On suppose que get_themes() peut être exécuté dans tous les cas, car c'est souvent basé sur le système de fichiers.
        if (!$this->app->runningInConsole()) {
            $themes = get_themes();
            foreach ($themes as $theme) {
                // Theme sources.
        ...
        // Le Gate::before peut aussi échouer si le modèle User n'est pas chargé
        // correctement ou si le système de rôles utilise la DB avant la migration.
        // Il est souvent plus sûr de le conditionner.
        if (!$this->app->runningInConsole()) {
            Gate::before(
                function ($user, $ability) {
                    return $user->hasRole('Super Admin') ? true : null;
